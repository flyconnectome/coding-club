...
data %>% filter(hemibrain_type %in% "ORN_DA1") -> ORN_DA1
```
...
ORN_DA1$root_id -> ids
flywire_partner_summary2(ids, partners = "outputs") -> outputs
```
...
outputs %>%  group_by(top_nt) %>% summarise(counts = sum(weight)) %>% arrange(desc(counts)) -> counts
```

##second order neurons
```{r}
#get the unique ids
unique(outputs$post_pt_root_id) -> ids2
as.character(ids2)
```

```{r}
#get the downstream partners
flywire_partner_summary2(ids2, partners = "outputs") -> outputs2
```

```{r}
#see if you have reached any DNs
outputs2 %>%  filter(grepl("^DN.+", hemibrain_type)) -> DNs
...
outputs2 %>%  group_by(top_nt) %>% summarise(counts = sum(weight)) %>%  arrange(desc(counts)) -> counts2
```
...
```{r}
outputs %>% filter(top_nt == "acetylcholine") -> a1 #find the first order neurons expressing acetylcholine
unique(a1$post_pt_root_id) -> a1
head(a1) -> a1 #just pick a few of them 
as.character(a1)
fafbseg::read_l2skel(a1) -> a1skel #get the l2 skeletons
```

```{r}
outputs %>% filter(top_nt == "gaba") -> g1 #find the first order neurons expressing gaba
unique(g1$post_pt_root_id) -> g1
head(g1) -> g1 #just pick a few of them 
as.character(g1)
fafbseg::read_l2skel(g1) -> g1skel #get the l2 skeletons
```

```{r}
outputs2 %>% filter(top_nt == "acetylcholine") -> a2 #find the second order neurons expressing acetylcholine
unique(a2$post_pt_root_id) -> a2
head(a2) -> a2 #just pick a few of them 
as.character(a2)
fafbseg::read_l2skel(a2) -> a2skel #get the l2 skeletons
```

```{r}
outputs2 %>% filter(top_nt == "gaba") -> g2 #find the second order neurons expressing gaba
unique(g2$post_pt_root_id) -> g2
head(g2) -> g2 #just pick a few of them 
as.character(g2)
fafbseg::read_l2skel(g2) -> g2skel #get the l2 skeletons
```
